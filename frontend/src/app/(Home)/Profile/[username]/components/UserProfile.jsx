...
const handleAddFriend = async (loggedInUser, user) => {
  try {
    const respond = await axios.post(
      `https://${HOST}/api/friend/friends-add/`, 
...
const handleBlockUser = async (loggedInUser, user) => {
  try {
    const respond = await axios.post(
      `https://${HOST}/api/friend/friends-remove/`,
      { username: user.username },
      { withCredentials: true, headers: {} }
    );
...
function Part1({loggedInUser, user, isSelf}) {
  ...
  return (
    ...
    <div className="flex flex-col items-center">
        <img
            src={user?.image_field? `https://${HOST}/api/api/images/${user.image_field}` : "/images/DefaultAvatar.svg"} // image_feiled
            alt="ProfileImage"
            width={60}
            height={60}
    ...
    const fetchXp = async () => {
      try {
        const [normalMatchesResponse, aiMatchesResponse] = await Promise.all([
          axios.get(`https://${HOST}/api/game/fetch_history/${user.username}/`),
          axios.get(`https://${HOST}/api/game/matches/${user.username}/`)
        ]);
