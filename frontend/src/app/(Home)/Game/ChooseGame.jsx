...
function MainComponent() {
  ...
  useEffect(() => {
    const fetchFriends = async () => {
      try {
        const response = await customAxios.get(`https://${HOST}/api/friend/friends`);
        if (response.data) {
  ...
  useEffect(() => {
    ...
    const searchPar = async () => {
      ...
      if (showTournamentParam === 'true' && tournamentRoomParam) {
        const bracketUpdate = await customAxios.get(`https://${HOST}/api/game/bracket/${tournamentRoomParam}/`); 
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          <img
                            src={friend.user.image_field ? `https://${HOST}/api/api${friend.user.image_field}` : "/images/Default_profile.png"}
